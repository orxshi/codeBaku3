...
Iblank::Iblank ()
{
    ...
    if (in.is_open())
    {
        ...
        // read hole cutting method if prescribed.
    }
...
void Iblank::identify (Grid& grAct, Grid& grPas)
{
    ...
    if (holeCutter.holeCuttingMethod == holeCutter.holeCutter_t::EXPLICIT_AABB)
    {
        if (grPas.holes.size() > 0)
        {
            holeCutter.explicit_AABB (grAct, grPas.holes[0].min, grPas.holes[0].max);
        }
